...
```
[x] 1. Rename v2025_12_04.rs → v2025_12.rs
[x] 2. Add PathStyle, SpecVersion to version.rs
[x] 3. Update ManifestVersion enum (remove V2025_12_04_beta, add V2025_12)
[x] 4. Update Manifest enum to include SpecVersion
[x] 5. Create manifest_types.rs with newtype wrappers
[x] 6. Update encode.rs to use new spec strings
[x] 7. Update decode.rs to parse new spec strings
[x] 8. Update all tests referencing V2025_12_04_beta
[x] 9. Update storage content-type headers
...
### Phase 1: Model Updates ✅ DONE
- [x] Remove `V2025_12_04_beta`, add `V2025_12` with `SpecVersion`
- [x] Rename `v2025_12_04.rs` → `v2025_12.rs`
- [x] Add `PathStyle`, `SpecVersion` to `version.rs`
- [x] Create `manifest_types.rs` with newtype wrappers
- [x] Update encode/decode for new spec strings
- [x] Update all tests

### Phase 2: Operations Module ✅ DONE
- [x] Create `crates/model/src/operations/` module
- [x] Implement `compute_diff_manifest()` (manifest→manifest)
- [x] Implement `compose_manifests()` with trie-based merging
- [x] Implement `apply_diffs()` for applying diffs to snapshots
- [x] Implement `filter_manifest()` with `IncludeExcludeFilter`
- [x] Implement `subtree_manifest()`
- [x] Implement `partition_manifest()` with auto-detect roots
- [x] Implement `join_manifest()` (inverse of subtree)
- Note: `collect_manifest()` and `hash_manifest()` remain in filesystem crate

### Phase 3: Storage Updates ✅ DONE
- [x] Add `ContentAddressedDataCache` trait to `traits.rs`
- [x] Add `S3DataCache` implementation with optional `S3CheckCache`
- [x] Add `FileSystemDataCache` implementation
- Note: Orchestrator method renames deferred (non-breaking)

### Phase 4: Filesystem Updates ✅ DONE
- [x] Add `SymlinkPolicy` enum (6 variants) to `symlink.rs`
- [x] Add helper methods: `should_follow()`, `should_exclude()`, `should_preserve()`, `should_include_targets()`
- Note: Scanner's `SnapshotOptions` still uses `follow_symlinks: bool` - full integration deferred

### Phase 5: Cleanup ✅ DONE
- [x] All tests pass (94 model, 99 storage, 85 filesystem)
- [x] cargo fmt clean
- [x] cargo clippy clean (no warnings)
- [x] Code reviewed with code-simplifier principles
