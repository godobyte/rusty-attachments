...
**Status:** Partially implemented (CLI in `crates/cli/`, path utilities in `crates/common/`)
...
The `ra` CLI tool implements manifest and attachment operations:

```
ra manifest snapshot  — Create snapshot manifest from directory
ra manifest diff      — Diff directory against existing manifest
ra attachment download — Download files from manifest via S3
ra attachment upload   — Upload files from manifest to S3
ra config             — Configuration management
```

Uses clap for argument parsing, tokio for async, and composes storage/filesystem crate primitives.
